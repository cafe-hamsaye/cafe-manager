    ...
    <div class="bg-surface border border-border-subtle rounded-2xl shadow-xl p-8">
      ...
      <form @submit.prevent="handleSubmit" class="space-y-6">
        ...
        <div>
          ...
          <input v-model="phoneNumber" type="tel" id="phoneNumber-signup" class="w-full p-3 bg-input-bg border border-border-subtle rounded-lg focus:border-action focus:ring-2 focus:ring-action/20 outline-none transition-all duration-300" placeholder="09123456789" required>
        </div>

        <div>
          <label for="password-signup" class="block text-sm font-medium text-heading mb-2">رمز عبور</label>
          <div class="relative">
            <input v-model="password" :type="passwordFieldType" id="password-signup" class="w-full p-3 bg-input-bg border border-border-subtle rounded-lg focus:border-action focus:ring-2 focus:ring-action/20 outline-none transition-all duration-300 pr-10" placeholder="••••••••" required>
